 ...
 *
 * @version     1.0.0
 * @package     Charitable PayPal Express/Classes/Recurring Donations
 * @author      Eric Daams
 * @copyright   Copyright (c) 2017, Eric Daams
 * @license     http://opensource.org/licenses/gpl-3.0.php GNU Public License
 */

namespace Charitable_PayPal_Express\Recurring;

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) { exit; }

/**
 * PayPal Express Payment Gateway support for recurring donations.
 *
 * @since       1.0.0
 */
class Recurring_Support {
	...
	public function maybe_process_recurring_donation( $return, $donation_id, Charitable_Donation_Processor $processor ) {
		 ...
		 */
		remove_filter( 'charitable_process_donation_paypal-express', array( '\Charitable_PayPal_Express\Gateway\PayPal_Express', 'process_donation' ), 10, 3 );
		...
		$gateway 	  = new \Charitable_PayPal_Express\Gateway\PayPal_Express();
	...
	public function create_recurring_donation_plan( $return, $campaign_id, $plan_args, $args ) {
		...
		if ( version_compare( charitable_recurring()->get_version(), '1.0.5', '<' ) ) {
			...
			$amount = Charitable_Gateway_Stripe::get_amount( $plan_args['amount'], $currency );
		}
